...
def inference(model, x):
    ...
    bpp = sum(len(s[0]) for s in out_enc["strings"]) * 8.0 / num_pixels # s是ystring(or zstring) s[0]表示取ystring这个list的第一个元素
...
def eval_model(model, filepaths, entropy_estimation=False, half=False):
    ...
    for f in filepaths:
        ...
        # print('mxh eval_model() Input image tensor shape: ' + str(x.shape)) # (C,H,W)(3,512,768)
        if not entropy_estimation:
            if half:
                model = model.half()
                x = x.half()
            rv = inference(model, x)
            print(f"actual bpp:{rv['bpp']}")
            # rv = inference_entropy_estimation(model, x)
            # print(f"estimate bpp:{rv['bpp']}")
            # print("----------------------------------")
        else:
            rv = inference_entropy_estimation(model, x)
        for k, v in rv.items():
